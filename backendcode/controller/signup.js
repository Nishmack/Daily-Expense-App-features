  ...
  try {
    ...
    if (name === "" || email === "" || password === "") {
      return res.status(400).json({
        success: false,
        message: "please fill all the details of the form",
      });
    }
    ...
    const uniqueEmail = await signUpUserDetails.findAll({
      where: { email: email },
    });
    ...
    if (uniqueEmail.length !== 0) {
      ...
      return res.status(500).json({
        success: false,
        message: "user already exist,change the Email",
      });
    }
    /* 
    ...
    bcrypt.hash(password, saltrounds, async (err, hash) => {
      ...
      const data = await signUpUserDetails.create({
        name: name,
        email: email,
        password: hash, //hashed pw stored in the db
      });
    });

    res.json({
      success: true,
      message: "Signup succesfull,login to enter a page",
    });
  } catch (error) {
    console.log("error from adduserdb", error);
    res.json({
      success: false,
      message: "user already exist..please signup with new email",
    });
  }
};
